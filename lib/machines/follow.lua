...
function machine:draw_grid(layer, momentary)
	g:led(10, 7, basic_lighting(layer.follow_strum==1))
...
function machine:grid_key(layer, momentary)
    local delta = 0
	self.follow_strum = momentary[10][7] == 1 and true or false
	...
	elseif self.follow_strum then layer:invert_follow_strum()
	end
...
function machine:get_current_gesture()
    if self.interval_decrease or self.interval_increase or self.interval_decrease_7 or self.interval_increase_7 then
		return "+/- interval"
	elseif self.follow_strum then return "follow strum"
	end
end
...
function machine:process(event)
	...
	if event.layer.notes_send == 1 then
		local routing = #event.layer.leader_routing > 0 and event.layer.leader_routing or {event.lane}
		event.offset = event.layer.follow_offset
		for i=1,#routing do
			harvest(routing[i], event, event.layer.follow_strum==1)
		end
	end
end
