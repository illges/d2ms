...
function trig:draw_screen(layer)
	...
	screen.move(1, 52)
	local action = "toggle"
	if layer.layer_mute == 1 then action = "sus1"
	elseif layer.layer_unmute == 1 then action = "sus2"
	end
			...
			for j=1,5 do
				if event.layer.input_machine[i].layer[j] == 1 then
					if event.layer.layer_mute_toggle==1 then
						machine_context.input[i].machine[j]:set_hold_toggle()
					elseif event.layer.layer_mute==1 then
						machine_context.input[i].machine[j]:set_hold_toggle(0)
					else
						machine_context.input[i].machine[j]:set_hold_toggle(1)
					end
				end
			end
		end
