...
function trig:process(event)
	...
	if event.layer.group_advance == 1 then
		notes_context.lane[event.lane].pattern_lane:adv_chain(val)
		notes_context.lane[event.lane]:update_active_pattern(notes_context.lane[event.lane].pattern_lane:range_min())
	else
		if val>0 then
			notes_context.lane[event.lane]:update_active_pattern(val)
		else
			notes_context.lane[event.lane]:advance_pattern_seq()
		end
	end
end
