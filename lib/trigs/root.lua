...
function trig:draw_screen(layer)
	...
	local val = layer.root_sequence[layer.position]
	screen.text("n"..position..":"..music.note_num_to_name(val))
end
...
function trig:grid_key(layer, momentary)
	...
	if delta ~= 0 then
		...
		if num > 0 then
			layer:set_root(num, layer.root_sequence[num] + delta)
		end
	end
end
...
function trig:process(event)
	...
	if layer.vel_mode == 1 then
		...
		notes_context.piano:set_root(event.layer.root_sequence[event.layer.position])
	else
		notes_context.piano:set_root(event.layer.root_sequence[event.layer.position])
